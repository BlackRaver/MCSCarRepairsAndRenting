...
const apiService = {
    ...
    deleteClient: async (id) => {
        if (USE_MOCKS) {
            window.mockData.clients = window.mockData.clients.filter(c => c.id !== id);
...
saveClientContacts: async (clientId, contacts) => {
    ...
    deleteVehicle: async (id) => {
        if (USE_MOCKS) {
            window.mockData.vehicles = window.mockData.vehicles.filter(v => v.id !== id);
            return Promise.resolve(true);
        }
    ...
    deleteOrder: async (id) => {
        if (USE_MOCKS) {
            window.mockData.orders = window.mockData.orders.filter(o => o.id !== id);
            return Promise.resolve(true);
        }
